...
Static Function PrintReport(oReport)
	...
	#IFDEF TOP	//Transforma parametros do tipo Range em expressao SQL para ser utilizada na query 	
	MakeSqlExpr("MTR530")	
	oSection:BeginQuery()	
	If ( mv_par03 == 1 )		
	...
	BeginSql alias "QRYSA1"		
		...
		AND			  A1_TEMVIS > 0 
		AND SA1.%notDel% %exp:cPart%		
		ORDER BY A1_VEND	
	EndSql	/*	Prepara relatorio para executar a query gerada pelo Embedded SQL passando como 	parametro a pergunta ou vetor com perguntas do tipo Range que foram alterados 	pela funcao MakeSqlExpr para serem adicionados a query	*/	
	oSection:EndQuery(mv_par04)
	#ELSE	//Transforma parametros do tipo Range em expressao ADVPL para ser utilizada no filtro	MakeAdvplExpr("MTR530")	//Adiciona no filtro o parametro tipo Range	If !Empty(mv_par04)		cFiltro += mv_par04 + " .AND. "	EndIf	cFiltro += " A1_TEMVIS > 0 "	If ( mv_par03 == 1 )		cFiltro += ' .AND. ('+DtoC(dDataBase)+'-A1_ULTVIS) > A1_TEMVIS'	
	EndIf	
	oSection:SetFilter(cFiltro,"A1_VEND")		TRPosition():New(oReport:Section(1),"SA3",1,{|| xFilial() + SA1->A1_VEND})	
	#ENDIF	
	oSection:Print()
